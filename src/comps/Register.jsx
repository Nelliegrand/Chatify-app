...
function Register() {
    ...
    const handleRegister = async () => {
        try {
            ...
            if (response.status === 201) {
                setSuccess("Registrering lyckad, var god logga in på nästa sida!");
    ...
    return (
        <>
            <Header />
            <div>
                <h1 className="new-reg">Registrera nytt konto</h1>
                {error && <p className="error-message">{error}</p>}
                {success && <p className="success-message">{success}</p>}
                <h2 className="pick-avatar">Välj en Avatar</h2>
                <div className="avatar-container">
                    {avatars.map((url, index) => (
                        <img
                            ...
                            src={url}
                            alt={`Avatar ${index + 1}`}
                            className={`avatar-image ${avatarUrl === url ? 'selected' : ''}`}
                ...
                <div className="form-container">
                    ...
                    <button className="btn-primary-register-login" onClick={handleRegister}>Registrera</button>
                </div>
            </div>
            <Footer />
        </>
